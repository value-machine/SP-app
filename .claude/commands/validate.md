...
**Related:** Create plans with `.claude/commands/plan.md`. Execute with `.claude/commands/implement.md`. Small scoped PIV in one go: `.claude/commands/quick-piv.md`. Session context: `.claude/commands/prime.md`. Commits and changelog: `.claude/commands/finish.md`.
...
- `CLAUDE.md` and relevant rules: `CLAUDE.md`, `CLAUDE.md`, `CLAUDE.md`, `CLAUDE.md`, `CLAUDE.md`, `CLAUDE.md`, `CLAUDE.md` (as applicable to the plan).
...
- **Consistency:** Steps match Conflict & compliance; layer boundaries and import direction described correctly; complexity expectations align with `CLAUDE.md`.
...
- **Architecture:** Spot-check imports against `CLAUDE.md` and `.dependency-cruiser.cjs` (especially new cross-layer paths).
- **Security / DB:** If the plan touched auth, RLS, or migrations, cross-check `CLAUDE.md` and `CLAUDE.md`.
...
| File placement | `CLAUDE.md` → `projectStructure.config.cjs` |
...
4. If the user wants fixes: apply them (plan markdown and/or source). Changelog remains the responsibility of `.claude/commands/finish.md` unless the user explicitly includes it in the requested fixes.
