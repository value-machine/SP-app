...
**Related:** For session context, use `.claude/commands/prime.md`. For requirement gates before building, use `.claude/commands/check.md`. For small scoped work without a full plan file, use `.claude/commands/quick-piv.md`. To execute this plan phase by phase, use `.claude/commands/implement.md`. To review the plan or resulting code without changing anything first, use `.claude/commands/validate.md`. For commits and changelog, use `.claude/commands/finish.md`.
...
- [ ] Identify relevant rules from `.cursor/rules/` (start at `CLAUDE.md`).
...
**Naming:** Follow `CLAUDE.md`: folder `temp_job_<descriptive-name>/` (kebab-case, descriptive). Allowed by `projectStructure.config.cjs` under `documentation/jobs/` (nested `*.md` files).
...
|-------|----------|
| Overview | `CLAUDE.md` |
| Architecture | `CLAUDE.md` – layers, import direction, path aliases, structure whitelist |
| File placement | `CLAUDE.md` and `projectStructure.config.cjs` |
| Code style | `CLAUDE.md` – naming, complexity (≤10 cyclomatic, ≤15 cognitive, ≤100 lines per function where applicable) |
| Database | `CLAUDE.md` – migrations, idempotent, safe patterns |
| Security | `CLAUDE.md` – auth, RLS, validation, secrets |
| Testing | `CLAUDE.md` |
| Workflow | `CLAUDE.md` – branch strategy (changelog only in finish) |
| Cloud / Edge | `CLAUDE.md` – when applicable |
| Debugging | `CLAUDE.md` – when diagnosing complex issues |
...
- [ ] Estimate complexity (cyclomatic ≤10, cognitive ≤15, functions ≤100 lines per `CLAUDE.md`); plan extractions if needed (see `.claude/commands/optimize.md` for refactoring workflow).
...
- [ ] Consider testing: new or changed logic should have tests per `CLAUDE.md`.
- [ ] Workflow: branch strategy per `CLAUDE.md` (e.g. not developing on `main`). Changelog is updated in **finish**, not while planning.
