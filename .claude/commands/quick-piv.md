...
**Critical:** Still follow repo rules. No changelog updates (`.claude/commands/finish.md` handles that).

**Related:** Full formal cycle: `.claude/commands/plan.md` → `.claude/commands/implement.md` → `.claude/commands/validate.md`. Session context: `.claude/commands/prime.md`. Requirement depth before coding: `.claude/commands/check.md`.
...
2. Quick investigation: search codebase; skim `CLAUDE.md` for applicable rules; use `documentation/DOC_TANSTACK_QUERY.md` if server state / queries are involved.
3. Sanity-check file placement against `CLAUDE.md` and `projectStructure.config.cjs`.
...
3. Follow `CLAUDE.md`, `CLAUDE.md`, `CLAUDE.md`, and `CLAUDE.md` when relevant.
...
6. **If failures:** Ask: “Fix these? (all / specific / skip)” — only fix after the user chooses (same spirit as `.claude/commands/validate.md`, compressed).
...
**Also when applicable:** `CLAUDE.md`, `CLAUDE.md`, `CLAUDE.md`, `ARCHITECTURE.md`, `documentation/DOC_TANSTACK_QUERY.md`.
