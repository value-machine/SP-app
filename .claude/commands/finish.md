...
- **If this was a debugging session that resulted in a fix:** update `.claude/commands/debug.md` § "Common Error Pattern Recognition" **only** when the insight is reusable.
...
- **MANDATORY:** Update changelog (fetch date if unsure of date) - see `CLAUDE.md` for Keep a Changelog format
...
- After `finish` creates the commit(s), run `.claude/commands/push.md` to handle remote verification and push.
...
**Note:** Commit message must match changelog entry. Husky pre-commit hook runs automatically (see `.husky/pre-commit` for SSOT). See `CLAUDE.md` for Keep a Changelog format details and version synchronization requirements.
