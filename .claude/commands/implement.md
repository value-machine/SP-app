...
**Related:** Create or refresh plans with `.claude/commands/plan.md`. For small tasks in one pass, `.claude/commands/quick-piv.md`. After phases, use `.claude/commands/validate.md` for a rules-and-tooling review before finish. For session context, `.claude/commands/prime.md`. For commits and changelog, `.claude/commands/finish.md`.
...
2. **Execute** the steps; match file layers, aliases, and patterns in `CLAUDE.md` and `ARCHITECTURE.md`.
...
| Topic | Location |
|-------|----------|
| Architecture | `CLAUDE.md` – layers, import direction, path aliases |
| File placement | `CLAUDE.md` and `projectStructure.config.cjs` |
| Code style | `CLAUDE.md` – naming, complexity (≤10 cyclomatic, ≤15 cognitive, ≤100 lines per function) |
| Database | `CLAUDE.md` – migrations |
...
If the plan requires doc updates (e.g. `src/features/<feature>/README.md`, `ARCHITECTURE.md`), do them in the phase that introduces the behavior. Follow the plan and `CLAUDE.md`.
